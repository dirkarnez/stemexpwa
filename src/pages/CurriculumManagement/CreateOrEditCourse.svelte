			...
			newIndexedArray(levelLength).forEach(i => {
				...
				newIndexedArray(lessonLength).forEach(j => {
					...
					} else
					{
						...
						formData.set(`${curriculumFormKeys.levels_key}.${i}.${curriculumFormKeys.lessons_key}.${j}.${curriculumFormKeys.lesson_presentation_notes_key}.0.presentation_note_file`, 
						new File([formData.get('levels.0.lessons.0.presentation_notes.0.presentation_note_file')], "345"))
					}
